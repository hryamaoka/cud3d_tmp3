...
#### コード行別解説 (`set_direction`)

| 行番号 | コード | 詳細解説 |
| :--- | :--- | :--- |
| 16-20 | `if (game->player.direction == 'N')` | **北向き設定**<br>`dir_y = -1` (画面上方向)。カメラ平面 `plane` は方向ベクトルと垂直になります。 |
| 21-25 | `else if (... == 'S')` | **南向き設定**<br>`dir_y = 1` (画面下方向)。 |
| 26-30 | `else if (... == 'W')` | **西向き設定**<br>`dir_x = -1` (画面左方向)。 |
| 31-35 | `else if (... == 'E')` | **東向き設定**<br>`dir_x = 1` (画面右方向)。 |
...
#### コード行別解説 (`set_player_info`)

| 行番号 | コード | 詳細解説 |
| :--- | :--- | :--- |
| 41-42 | `game->player.x = x + 0.5;` | **初期位置補正**<br>グリッド座標(整数)に0.5を足すことで、プレイヤーがマスの中心に配置されるようにします。これがないと壁の境界線上に配置されてしまう可能性があります。 |
| 43 | `game->player.direction = dir;` | キャラクタ文字（NSEW）を一時保存します。 |
| 44 | `set_direction(game);` | 向きに基づいたベクトル計算を行います。 |

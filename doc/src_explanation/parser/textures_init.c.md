...
#### コード行別解説 (`init_textures`)

| 行番号 | コード | 詳細解説 |
| :--- | :--- | :--- |
| 36-43 | `if (!load_texture(game, 0, game->tex.no))` | **壁テクスチャ読み込み**<br>`.cub` ファイルで指定されたパス（NO, SO, WE, EA）からテクスチャをロードします。 |
| 45-52 | `if (!load_texture(game, -1, ...))` | **ボーナス用テクスチャ**<br>ドアやスプライトアイテムなど、プログラムで固定指定されているアセットをロードします。 |
| 54 | `return (1);` | 全て成功した場合に `1` を返します。 |
